namespace StockSharp.Algo.Storages
{
	...
	/// </summary>
	public class fxLocalMarketDataDrive : BaseMarketDataDrive
	{
		private sealed class fxLocalMarketDataStorageDrive : IMarketDataStorageDrive
		{
			...
			public fxLocalMarketDataStorageDrive( DataType dataType, string path, StorageFormats format, fxLocalMarketDataDrive drive )
			{
			...
			private readonly fxLocalMarketDataDrive _drive;
		...
		private readonly SynchronizedDictionary<Tuple<SecurityId, DataType, StorageFormats>, fxLocalMarketDataStorageDrive> _drives = new SynchronizedDictionary<Tuple<SecurityId, DataType, StorageFormats>, fxLocalMarketDataStorageDrive>();

		/// <summary>
		/// Initializes a new instance of the <see cref="fxLocalMarketDataDrive"/>.
		/// </summary>
		public fxLocalMarketDataDrive() : this( Directory.GetCurrentDirectory() )
		{
		}
		...
		public fxLocalMarketDataDrive( string path )
		{
		...
		public override IMarketDataStorageDrive GetStorageDrive( SecurityId securityId, DataType dataType, StorageFormats format )
		{
			...
			return _drives.SafeAdd( Tuple.Create( securityId, dataType, format ),
				key => new fxLocalMarketDataStorageDrive( dataType, GetSecurityPath( securityId ), format, this ) );
		}
