namespace StockSharp.Algo
{
    ...
    /// </summary>
    public class fxHistoryMessageAdapter : MessageAdapter
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="fxHistoryMessageAdapter"/>.
		/// </summary>
		/// <param name="transactionIdGenerator">Transaction id generator.</param>
		/// <param name="securityProvider">The provider of information about instruments.</param>
		public fxHistoryMessageAdapter( IdGenerator transactionIdGenerator, ISecurityProvider securityProvider )
		...
		private void Start( DateTimeOffset startDate, DateTimeOffset stopDate )
		{
			...
			Action dataLoading = ( ) =>
			{
				try
				{
                    /* -------------------------------------------------------------------------------------------------------------------------------------------
                     * 
                     *  Tony 12 : To support multiTimeframe databar backtesting, I think this is where we need to modify the code to have databars send according to
                     * 
                     * ------------------------------------------------------------------------------------------------------------------------------------------- */
					...
					while( !IsDisposed && !token.IsCancellationRequested )
					{
						...
						DateTimeOffset loadDate   = _currentTime != DateTimeOffset.MinValue ? _currentTime.UtcDateTime.Date : startDate.UtcDateTime;
						...
						while( loadDate.Date <= stopDate.Date && !_isChanged && !token.IsCancellationRequested )
						{
							...
							loadDate = loadDate.UtcDateTime.AddDays( 1 );
						}

						if( !_isChanged )
						{
							SendOutMessage(
												new EmulationStateMessage
												{
													LocalTime = stopDate,
			...
			ThreadingHelper.ThreadInvariant( dataLoading ).Name( Name ).Launch();
		}
		...
		* ------------------------------------------------------------------------------------------------------------------------------------------- */
		private void EnqueueMessages( DateTimeOffset startDate, DateTimeOffset stopDate, DateTimeOffset loadDate, DateTimeOffset startTime, CancellationToken token, IEnumerable<Message> messages )
		{
			...
			foreach ( var msg in messages )
			{
				...
				DateTimeOffset openTime = serverTime;
				DateTimeOffset closeTime = serverTime;

				if ( msg is TimeFrameCandleMessage a )
				{
					var period = a.TimeFrame;

					if ( period >= TimeSpan.FromDays( 1 ) )
					{
						closeTime = openTime.UtcDateTime + period;
					}


					//var openTime = CandleHelper.GetFxcmBarOpenTimeBy( serverTime.UtcDateTime, a.TimeFrame );
					closeTime = CandleHelperEx.GetFxcmBarCloseTime( closeTime.UtcDateTime, a.TimeFrame );
				...
				/* -------------------------------------------------------------------------------------------------------------------------------------------
                 * 
                 *  Tony: I want the databars to be sorted by the database Close time, we don't want the Daily Candle to be finished before the Smaller time frame bars
                 *  are delievered.
                 * 
                 * ------------------------------------------------------------------------------------------------------------------------------------------- */
				msg.LocalTime = closeTime;
		...
		{
			var orderedRanges = Boards.Where(b => b.IsTradeDate(date, true)).SelectMany(board =>
																						{
