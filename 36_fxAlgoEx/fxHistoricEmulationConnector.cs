...
namespace StockSharp.Algo.Testing
{
    ...
    /// </summary>
    public class fxHistoricEmulationConnector : BaseEmulationConnector
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="fxHistoricEmulationConnector"/>.
        /// </summary>
        ...
        public fxHistoricEmulationConnector( IEnumerable<Security> securities, IEnumerable<Portfolio> portfolios )
        ...
        public fxHistoricEmulationConnector( IEnumerable<Security> securities, IEnumerable<Portfolio> portfolios, IStorageRegistry storageRegistry )
            : this(
                        new CollectionSecurityProvider( securities ),
                        new CollectionPortfolioProvider( portfolios ),
                        storageRegistry.CheckOnNull().ExchangeInfoProvider,
                        storageRegistry
                  )
        {
        }
        ...
        /// <param name="portfolios">Portfolios, the operation will be performed with.</param>
        public fxHistoricEmulationConnector( ISecurityProvider securityProvider, IEnumerable<Portfolio> portfolios )
            : this(
                        securityProvider,
                        new CollectionPortfolioProvider( portfolios ),
        ...
        public fxHistoricEmulationConnector( ISecurityProvider securityProvider, IPortfolioProvider portfolioProvider, IExchangeInfoProvider exchangeInfoProvider )
            : this(
                        securityProvider,
                        portfolioProvider,
                        exchangeInfoProvider,
                        new StorageRegistry( exchangeInfoProvider )
                  )
        {
        }
        ...
        /// <param name="storageRegistry">Market data storage.</param>
        /// <param name="exchangeInfoProvider">Exchanges and trading boards provider.</param>
        public fxHistoricEmulationConnector( ISecurityProvider securityProvider, IPortfolioProvider portfolioProvider, IExchangeInfoProvider exchangeInfoProvider, IStorageRegistry storageRegistry )
            : this(
                        new fxHistoryMessageAdapter( new IncrementalIdGenerator(), securityProvider ) { StorageRegistry = storageRegistry },
                        true,
                        new InMemoryMessageChannel( new MessageByLocalTimeQueue(), "Emulator in", err => err.LogError() ) { SuspendMaxCount = int.MaxValue },
                        securityProvider,
                        portfolioProvider,
                        exchangeInfoProvider
                  )
        ...
        /// <param name="portfolioProvider">The portfolio to be used to register orders. If value is not given, the portfolio with default name Simulator will be created.</param>
        /// <param name="exchangeInfoProvider">Exchanges and trading boards provider.</param>
        public fxHistoricEmulationConnector( IMessageAdapter innerAdapter, bool ownInnerAdapter, IMessageChannel inChannel, ISecurityProvider securityProvider, IPortfolioProvider portfolioProvider, IExchangeInfoProvider exchangeInfoProvider )
            : base(
                        new EmulationMessageAdapter( innerAdapter, inChannel, true, securityProvider, portfolioProvider, exchangeInfoProvider ) { OwnInnerAdapter = ownInnerAdapter },
                        false,
                        false
                  )
        {
            ...
            // when testing by candlesticks, the time changes faster and the timeout should be more than 30s.
        ...
        /// </summary>
        public fxHistoryMessageAdapter HistoryMessageAdapter => EmulationAdapter.FindAdapter<fxHistoryMessageAdapter>();
